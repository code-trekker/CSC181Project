...
@app.route('/adminattendance', methods=['GET','POST'])
@login_required
def addattendance():
    form = NewAttendance()
    #db.createTable(Attendance)
    if request.method == 'POST' and form.validate_on_submit():
        attendDate = form.attendDate.data
        check = session.query(Attendance).filter_by(Event.attends.any(memberid=form.memberid.data,eventid=form.eventid.data)).first()
        #check = session.query(Attendance).filter(Member.event.any(Event.eventid==eventid, Member.memberid==memberid))
        if check is None:
            add2.member_attends.append(memberid)
            add2.member_attends.append(attendDate)
            #db.session.add()
            db.session.commit()
            msg = 'Student successfully added'
            return render_template('attendance.html', form=form, msg=msg)
        elif check.memberid == form.memberid.data:
            msgs = "Student already added!"
            return render_template('attendance.html', form=form, msg=msg)
