...
Attendance = db.Table('Attendance',
    db.Column('eventid', db.String(4), db.ForeignKey('event.eventid')),
    db.Column('memberid', db.Integer(), db.ForeignKey('member.memberid')),
    db.Column('attendDate', db.String(11), nullable=False)
    )
...
class Member(db.Model):
    ...
    orgCode = db.Column(db.String(4), nullable=True)
    Attendance = db.relationship('Event', secondary=Attendance, backref=db.backref('attends', lazy='dynamic'))
...
class Organization(db.Model):
    ...
    #attends = db.relationship('Attendance', backref='organization', lazy=True)
